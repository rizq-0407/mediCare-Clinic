...
export default function AdminDashboard() {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      if (isEditing) await API.put(`/schedules/${currentId}`, formData);
      ...
      setIsEditing(false); setCurrentId(null); fetchSchedules();
    } catch (err) {
      if (!err.response) {
        if (isEditing) setSchedules(prev => prev.map(s => s.id === currentId ? { ...formData, id: currentId } : s));
  ...
  const handleEmployeeRegistration = async (e) => {
    ...
    try {
      ...
      fetchUsers();
    } catch (err) { alert(err.response?.data?.message || "Employee registration failed."); }
    finally { setEmpIsLoading(false); }
  };
                        ...
                        <div key={user.id} className="soft-card" style={{ padding: '1.5rem', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                          <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
                            ...
                            <div>
                              ...
                              <span className={`badge ${user.role === 'ADMIN' ? 'badge-danger' : user.role === 'DOCTOR' ? 'badge-success' : 'badge-info'}`}>{user.role}</span>
                            </div>
                          </div>
                          <button className="btn btn-soft" style={{ padding: '0.5rem', color: 'var(--danger)' }} onClick={() => handleDeleteUser(user.id, user.username)}>
